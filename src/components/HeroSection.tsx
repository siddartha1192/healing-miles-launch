...
function StatCounter({ target, suffix, label }: { target: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref}>
      <div className="text-4xl font-display text-primary mb-1 tabular">
        {count}{suffix}
      </div>
      ...
      <div className="container mx-auto relative z-10">
        <div className="max-w-4xl">
          ...
          <div className="flex flex-wrap gap-4 mb-20">
            <button onClick={() => scrollTo("#consultation")} className="btn-primary flex items-center gap-2">
              Book 1:1 Consultation — ₹799
            </button>
            <button onClick={() => scrollTo("#webinar")} className="btn-outline">
              Join Free Webinar →
            </button>
          </div>
